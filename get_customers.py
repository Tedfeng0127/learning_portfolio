import requests             # 發出請求的模組
import json                 # 將python內部的資料型態dict轉換成json檔案，或者反之

# 原理:
# 我們(client)發出request給華南銀行的伺服器(server)，
# 華南銀行的server會辨別我們的request(也就是那一串url, 網址)，透過我們給予的參數(也就是api_key、uiid等)
# 辨識我們要什麼回應(response)，回應就是回傳給我們的資料(有各種不同形式，有時候是網頁，有時候是pdf)
# 而這個case的response是json檔案。(json檔案是網路傳輸資料的一種標準資料格式，資料規則簡單，需要的容量少，因此常常用以傳遞大量資料)


# 發出request，將華南銀行server的response存在response這個變數(這一步就是透過API得到資料)
# get是requests這個類別(class)裡面的一個函式，這個例子我們輸入兩個參數，一個是"url(地址)", 另一個是"headers"
...
# uuid_list 取得, 一次10筆
# 用response.text把response轉成text(這樣才可以在python裡面處理)(是固定用法，先記著!)
# 再用json這個類別裡面的函式loads把response.text的內容轉換成dict資料型態
