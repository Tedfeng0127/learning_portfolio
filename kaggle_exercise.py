...
# 所以一個隨機森林裡面的任兩個決策樹，它們用來的分類的特徵不一定會一樣對吧？
# 每一顆決策樹起始點的樣本都是從全部的資料裡面去隨機抽取出來的，而每顆樹中間在做分類的特徵也都是依據資料集裡面的特徵隨機選取的？
# 不容易有overfitting的問題，因為在建模的時候就有兩個隨機性(隨機抽樣、隨機選取分類特徵)？

# 一個隨機森林裡面有很多棵決策樹，隨機森林模型是一個集成式的模型，他等於是把這片隨機森林裡面所有決策樹的效果及合起來，
# 一棵決策樹模型會根據餵給他的資料去決定每個節點要用什麼特徵去split這筆資料，而每棵決策樹最後停止的葉節點裡面只有兩種可能的情況，
# 一種是資料已經被分到到達這個葉節點的時候只剩下一筆，沒辦法再分成兩類，另一種是現在這個葉節點裡的資料全部都是同一個類別或是同一個值，
# 所以利用決策樹模型預測資料的時候，這筆新的資料餵進去，它會從起始點開始根據每一個它遇到的節點往下走，最後走到的那個葉節點的類別或數值就是它的預測結果，
# 而隨機森林模型做預測的時候是把這筆新的資料同時餵給這片森林裡面的每棵樹，每棵樹都會給出一個針對這筆資料的預測結果，如果是分類問題的話，
# 會依據票數多寡，最多棵樹產生出來的結果就會是這個隨機森林模型預測出來的最終結果，如果是預測數值的問題，
# 就會根據每棵樹所佔的權重加權每棵樹預測出來的數值，最後加權算出來的數值就是這個隨機森林模型的預測結果。
